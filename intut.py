...
def process_frame(frame, outs, classes, confThreshold, nmsThreshold,fr):
    ...
    for i in indices:
        ...
        if classIds[i] in {3}:
            c=c+1
            bike_img, hh_img=cutout(frame, classes, classIds[i], confidences[i], max(0,left-(int((0)*width))), max(0,top-int((0.6*height))), min(frameWidth,left + int(1*width)), min(frameHeight,top + (height)),c,p,fr)
        # if classIds[i] in {0}:
        #     p=p+1
