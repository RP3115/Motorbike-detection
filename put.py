...
def uploader():
    direc=r"D:\IIT Mandi\DP\Minh\pol"
    os.chdir(direc)
...
def grouper():
    ...
    for root, _, files in os.walk("D:\IIT Mandi\DP\Minh\pol"):
            for f in files:
    ...
    image_dir = "D:\IIT Mandi\DP\Minh\pol"  # Replace with the path to the directory containing the images
...
def cutout(frame, classes, classId, conf, left, top, right, bottom,c,p,fr):
    ...
    if classId in {0}:
        ari=frame[top:bottom, left:right]
        if ari.size !=0:
            directory=r'D:\IIT Mandi\DP\Minh\pol'
            os.chdir(directory)
            filename = 'person{}_{}.jpg'.format(fr,p)
            cv2.imwrite(filename, ari)
            #print("done{}".format(c))
