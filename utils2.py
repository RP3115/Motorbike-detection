...
def process_frame(frame, outs, classes, confThreshold, nmsThreshold,fr):
    ...
    for i in indices:
        ...
        # if classIds[i] in {3}:
        #     c=c+1
        #     cutout(frame, classes, classIds[i], confidences[i], left-int((0.20*width)), top-int((0.5*height)), left + int((1.20*width)), top + (height),c,p,fr)
        #if classIds[i] in {0}:
        #     p=p+1
        #     cutout(frame, classes, classIds[i], confidences[i], left, top, left + width, top + height,c,p,fr)
        draw_prediction(frame, classes, classIds[i], confidences[i], left-int((0.25*width)), top-int((0.5*height)), left + int((1.25*width)), top + height)
